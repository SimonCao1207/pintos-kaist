...
__do_fork (void *aux) {
	...
	// Done duplication --> wake up thread
...
process_exec (void *f_name) {
	...
	*/
	...
	_if.rsp = _if.rsp - 8; // why ?
	memset(_if.rsp, 0, sizeof(char **));
	
	// Push the argument's address to the stack
	for (i=argc-1; i>=0; i--){
		_if.rsp -= 8;
...
process_wait (tid_t child_tid UNUSED) {
	...
	// while (1); // temporarily put infinite loop here.
...
process_exit (void) {
	...
	// Empty child list of current thread
	struct thread * child = NULL;
